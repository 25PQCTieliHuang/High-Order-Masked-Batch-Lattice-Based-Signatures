...
int batch_test()
{
    ...
    do {
        ...
        cmul = 0;
        for (i = 0; i < iter; i++) {
            cmul += crypto_sign(sm, &smlen, msg, mlen, sk);
        }
        cc = plat_get_cycle() - cc;
        ts = cpu_clock_secs() - ts;
    } while (ts < to);
    printf("%s\t  Sign() %5zu:\t%8.3f ms\t%8.3f Mcyc   Matrix-Matrix Multiplication(): \t%8.3f Mcyc\n", CRYPTO_ALGNAME, iter,
           1000.0 * ts / ((double)iter), 1E-6 * ((double) (cc / iter)),1E-6 * ((double) (cmul / iter)));
...
int main()
{
    ...
    printf("%s\t  Sign() %5zu:\t%8.3f ms\t%8.3f Mcyc\t%8.3f Mcyc\n", CRYPTO_ALGNAME, iter,
           1000.0 * ts / ((double)iter), 1E-6 * ((double) (cc / iter)),1E-6 * ((double) (cmul / iter)));
