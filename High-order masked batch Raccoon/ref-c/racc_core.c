...
static void	add_rep_noise(int64_t vi[RACC_D][RACC_N], int i_v, int u,
		mask_random_t *mrg)
{
...
uint64_t	racc_core_sign(racc_sig_t *sig, const uint8_t mu[RACC_MU_SZ],
		racc_sk_t *sk)
{
	...
	do
	{
		...
		for (i = 0; i < RACC_K; i++)
		{
			...
			{
				// cc = plat_get_cycle();
				polyr_ntt_cmul(mw[i][j], mr[0][j], ma[i][0]);
				for (k = 1; k < RACC_ELL; k++)
				{
					polyr_ntt_mula(mw[i][j], mr[k][j], ma[i][k], mw[i][j]);
				}
			...
			add_rep_noise(mw[i], i, RACC_UW, &mrg);
			//  --- 8.  w := Decode([[w]])
			racc_decode(vw[i], mw[i]);
			//  --- 9.  w := round( w )_q->q_w
...
bool	racc_core_verify(const racc_sig_t *sig, const uint8_t mu[RACC_MU_SZ],
		const racc_pk_t *pk)
{
	int64_t	aij[RACC_N];
	...
	for (i = 0; i < RACC_K; i++)
	{
		...
		polyr_shlm(u, pk->t[i], RACC_NUT, RACC_Q); 
		polyr_fntt(u);
		polyr_ntt_cmul(u, u, c_poly); 
...
uint64_t	racc_core_batch_sign(racc_sig_t *sig, const uint8_t mu[RACC_MU_SZ],
		racc_sk_t *sk)
{
	int64_t			ma[RACC_K][RACC_ELL][RACC_N];
	int64_t			mr[RACC_ELL][RACC_D][RACC_N];
	...
	for (size_t iter = 0; iter < RACC_BATCH_SIZE; iter++)
	{
        rsp = 0;
        for (i = 0; i < RACC_ELL; i++)racc_ntt_refresh(sk->s[i], &mrg);
		do
		{
			for (i = 0; i < RACC_ELL; i++)
			{
...
#if RACC_D >= 4||(RACCOON_128_2)

	csum +=racc_matrix_mul(mw,ma,mr);
		for (i = 0; i < RACC_K; i++)
				for (j = 0; j < RACC_D; j++) {
					polyr_intt(mw[i][j]);
			}
